...
export const list = async (req,res) => {
  try {
    ...
    res.status(500).json({ message: `Error to get list of clientType  ${e.message}` });
  }
}
...
export const getById = async (req,res) => {
  ...
  try {
    ...
    if(!result) return res.status(404).json({ success: false, message: 'ClientType no found'});
...
export const update = async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ success: false, message: 'Error undating ClientType'});
  }
};

// THERER NOT VERB DELETE...